...
1. **用户登录系统**
   ...
   - **UI 操作**：填写邮箱和密码 `Test@123456`，点击"登入"按钮

2. **进入语音管理模块**
   - 导航到"语音管理（组织树）"功能模块
   - **业务含义**：准备通过语音录音生成销售线索
   - **UI 操作**：点击左侧菜单中的"语音管理（组织树）"链接

### 第二阶段：线索生成与管理
3. **语音线索生成**
   - 点击"生成线索"按钮，将历史录音转化为销售线索
   - **触发 API**：`POST /flow/api/flow-rest/meeting-file-to-clue-flow` - AI 语音转线索流程
   - **UI 操作**：点击"新增"→"历史录音"→"生成线索"
   - **业务含义**：利用 AI 技术分析会议录音内容，自动提取潜在销售机会

4. **线索信息完善**
   - 进入线索管理页面，编辑线索详细信息
   - 补充区域信息：`华南`
   - 设置线索类型：`客户介绍`
   - **触发 API**：`PUT /form/api/v2/form-entity-data/19892/lead-management/lead-management-form/edit` - 更新线索数据
   - **UI 操作**：点击"编辑"按钮，填写区域和线索类型信息

### 第三阶段：客户创建
5. **新建客户档案**
   - 从线索中创建新客户：`自动化测试客户`
   - **触发 API**：`POST /form/api/v2/form-entity-data/customer-management/customer-management-form/default` - 创建客户记录
   - **UI 操作**：点击"新建客户"按钮，填写客户名称，点击"提交"
   - **业务含义**：将潜在线索转化为正式客户档案，建立客户关系
...
6. **线索转化为商机**
   ...
   - 设置预期成交金额：`1200` 元
   - 设置成功概率：`56%`
   - **触发 API**：`PUT /flow/api/v2/flow-definition/lead-management/opportunity-conversion-process-flow/lead-management-form/convert/update-form` - 线索转商机流程
   - **UI 操作**：点击"转化为商机"，填写商机名称、预期金额和成功概率
...
7. **创建销售报价**
   ...
   - **触发 API**：`PUT /flow/api/v2/flow-definition/quotation-management/new-quote-approval-flow/quotation-form/new/update-form` - 报价审批流程
   - **UI 操作**：在报价管理页面点击"新增"，填写报价信息和产品详情
...
10. **合同信息完善**
    ...
    - 设置乙方名称：`自动化测试乙方`
    - 完善合同条款和交付方式
    - **触发 API**：`PUT /form/api/v2/form-entity-data/19900/contract-management/contract-management-form/edit` - 合同信息更新
    - **UI 操作**：在合同管理页面点击"编辑"，填写乙方名称
